...
UniLab_GRAMMAR = r"""
    ...
    expr_stmt: expression (SEMI | COMMA)?
    
    ?assignment: multi_assignment | single_assignment
    multi_assignment.10: lhs_list EQUAL expression (SEMI | COMMA)?
    single_assignment: postfix_expr EQUAL expression (SEMI | COMMA)?
    ...
    clear_stmt: CLEAR (IDENTIFIER | "all")* (SEMI | COMMA)?
    global_stmt: GLOBAL IDENTIFIER+ (SEMI | COMMA)?
    return_stmt: RETURN (SEMI | COMMA)?
    break_stmt: BREAK (SEMI | COMMA)?
    continue_stmt: CONTINUE (SEMI | COMMA)?
    
    import_stmt: IMPORT qualified_name (SEMI | COMMA)?
    export_stmt: EXPORT IDENTIFIER (SEMI | COMMA)?
...
class UniLabToPython(Transformer):
    ...
    def multi_assignment(self, items):
        ...
        has_semi = str(items[-1]) == ";"
        stmt = f"({', '.join(lhs)}) = {expr}"
        return {"type": "assignment", "stmt": stmt, "lhs": f"[{', '.join(lhs)}]", "is_multi": True, "names": lhs, "has_semi": has_semi}
    ...
    def single_assignment(self, items):
        ...
        if "unilab_call" in str(lhs) or "unilab_get" in str(lhs):
            if "unilab_call" in str(lhs):
                ...
                if match:
                    ...
                    return {"type": "assignment", "stmt": stmt, "lhs": obj, "is_multi": False, "has_semi": has_semi}
        ...
        return {"type": "assignment", "stmt": f"{lhs} = {expr}", "lhs": str(lhs), "is_multi": False, "has_semi": has_semi}
    ...
    def expr_stmt(self, items):
        ...
        return {"type": "expr", "expr": expr, "has_semi": has_semi}
    ...
    def start(self, items):
        ...
        for i in items:
            ...
            if isinstance(i, list):
                ...
                res.append(self._process_stmt(i))
            else:
                s = str(i).strip()
                if s and s not in (";", ","): res.append(s)
        return "\n".join(res)
    ...
    def if_stmt(self, items):
        items = [i for i in items if i is not None and not (isinstance(i, str) and i.strip() in ("", ";", ","))]
    ...
    def transpile(self, code: str):
        ...
        try:
            ...
            if len(self._cache) >= self._cache_limit:
                # Remove a random item (good enough for 100 items)
