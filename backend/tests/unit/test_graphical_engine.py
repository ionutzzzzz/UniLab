...
async def test_graphical_engine():
    ...
    try:
        ...
        print("\n" + "="*50)
        print(" TEST 1: Standard plot() -> graph.png")
        print("="*50)
        res = await core.run_code(sid, "x = 0:0.1:10; y = sin(x); plot(x, y);")
        print(f"STDOUT:\n{res.stdout}")
        
        graph_path = pathlib.Path("./test_graphical") / session.workspace_path.name / "graph.png"
        if graph_path.exists():
            print(f"✅ Success: graph.png created at {graph_path}")
        else:
            print(f"❌ Error: graph.png NOT found at {graph_path}")
            
        print("\n" + "="*50)
        print(" TEST 2: terminal_plot() -> updates graph.png")
        print("="*50)
        ...
        if graph_path.exists():
            print(f"✅ Success: graph.png still exists (updated)")
